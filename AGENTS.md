...
Each home is also pinned to the terminal session and Claude account it was started from, so a home opened from another session refuses every fleet mutation instead of adopting that session's account; homes are never migrated or merged, and `bin/fm-home-identity.sh` owns that contract.

Tracked files hold shared instructions and tooling; `data/` holds durable private fleet records; `state/` holds runtime records and append-only status events; `config/` holds local operating choices; and `projects/` contains clones that are read-only to firstmate outside hard rule 1's named owner paths.
...
.env                 optional Relay pairing token (presence-gates section 14), mail-plane credentials (schema: docs/configuration.md "Mail plane"), and typed dispatch resolution key OPENROUTER_API_KEY (presence-gates bin/fm-dispatch-resolve.sh; docs/configuration.md "Typed dispatch resolution"); LOCAL, gitignored
...
config/claude-shaped-store  Claude credential stores whose firstmate-launched workers this home paces at a limit-reset edge, one absolute path per line; LOCAL, gitignored; absent means no store is shaped and every Claude launch is released unchanged; inherited by secondmate homes; see docs/configuration.md "Claude release shaping"
...
data/                personal fleet records; LOCAL, gitignored as a whole
  ...
  home-identity      this home's durable session and Claude-account pin; LOCAL, gitignored, written once by the first session that uses the home and never rewritten; every fleet mutation is refused from another session (bin/fm-home-identity.sh)
...
state/               runtime records and signals; gitignored
  ...
  claude-admission.check.sh  generated Claude release-shaping poll shim and its .check-trust binding; present only after bin/fm-claude-admission.sh arm; silent unless a shaped store has waiting work whose release slot is open, or its evidence is unreadable and that waiting work cannot be released at all; each distinct evidence problem is reported once, and a different or recurring one reports again
...
Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON at that intake, join one environment-pinned snapshot per enabled named-account store in the candidate set, use the skill's bounded unknown-pool warm-up and quota retry when needed, establish complete limit information for every candidate before any ranking or hand-picking, and rank by inspectable `spendPriority` across the union under the skill's eligibility, reasoning-class, runway, split, and Claude-reserve procedure.
...
A missing authentication source is not proof of failed login; a quota unknown triggers the skill's gather procedure and makes the whole choice incomplete, never drops a candidate from the choice, never treats unknown as healthy, and never implies failed authentication.
Only concrete contradictory evidence blocks a candidate, such as an authoritative catalog proving the model unsupported or proof that the credential selected for that surface is unusable; never infer a credential store, provider family, or quota mapping from a harness, model, or source name, and never launch another harness's CLI to judge a candidate (the skill's named pool warm-up is the sole exception for opening a quota window).
Preserve malformed profile configuration as an actionable error rather than selecting around it.
A named-account candidate is eligible only when this home's `config/accounts.json` explicitly enables cross-account routing and allowlists its account; an off switch makes it ineligible at the next intake, never moves an in-flight worker, and a Geris-only registry cannot grant personal seats.
...
When a `harness=claude` candidate's credential store is listed in `config/claude-shaped-store`, also load `quota-array-dispatch` before treating that store's headroom as available; it owns how the committed-demand census is read beside quota data and never folded into it.
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight; its batched Jev rule and candidate/effort Choices never bypass the account switch or quota gates, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override; `incomplete`, `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
...
For any review or merge ask, repeat the full URL in MAIN's final captain-facing response even if an earlier status or transcript already showed it.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap or network-checks section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `PRESENTATION_UNAVAILABLE:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `HOME_IDENTITY:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `NETWORK_CHECKS:`, `HOME_SUMMARY:`, `BACKLOG_RECONCILE:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `SECONDMATE_HANDOFF:`, `NUDGE_SECONDMATES:`, or `FMX:`), or when `BOOTSTRAP_INFO:` says an interrupted backlog cleanup may have left an endpoint or local copy; silence and other `BOOTSTRAP_INFO:` facts need no load.
- `diagnostic-reasoning` - load before scoping a reported bug and before acting on a diagnostic report.
- `ask-user-authority` - load before deciding any ask-user finding.
- `quota-array-dispatch` - load before choosing among a matched crew-dispatch profile array from per-store quota-axi snapshots, and before treating the headroom of a Claude credential store listed in `config/claude-shaped-store` as available.
