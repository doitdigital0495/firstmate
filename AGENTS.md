...
Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON at that intake, use the skill's bounded unknown-pool warm-up and quota retry when needed, and rank by inspectable `spendPriority` under the skill's eligibility, reasoning-class, runway, split, and Claude-reserve procedure.
...
- `quota-array-dispatch` - load before choosing among a matched crew-dispatch profile array from current quota-axi default TOON, and before treating the headroom of a Claude credential store listed in `config/claude-shaped-store` as available.
