...
Firstmate alone resolves a matched profile array: begin with `quota-axi`'s default TOON at that intake, join one environment-pinned snapshot per enabled named-account store in the candidate set, use the skill's bounded unknown-pool warm-up and quota retry when needed, and rank by inspectable `spendPriority` across the union under the skill's eligibility, reasoning-class, runway, split, and Claude-reserve procedure.
...
A missing authentication source is not proof of failed login; a quota unknown triggers the skill's one-retry procedure and can drop only its affected candidate from this choice, not block the whole choice or imply failed authentication.
...
Run `bin/fm-dispatch-resolve.sh` directly on the written brief in the same turn, with no preflight; its batched Jev rule and candidate/effort Choices never bypass the account switch or quota gates, and on `clear` pass its `profile:` line to `fm-spawn` unless you state a reason to override; `ambiguous`, `escalate`, `error`, and off all mean the intake above, unchanged (contract: `docs/configuration.md` "Typed dispatch resolution").
