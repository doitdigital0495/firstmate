...
{"rules":[{"when":"A simple bug fix with a stated root cause.","use":[
 {"harness":"codex","model":"gpt-5.6-sol","account":"geris"},
 {"harness":"pi","model":"openai-codex/gpt-5.6-sol","provider":"codex","account":"personal"}]}],
 "default":[{"harness":"claude","model":"opus"}]}
JSON
case $SCEN in
  declared) PLANS_P='"plans":{"claude":"Max 20x","codex":"ChatGPT Pro"},' PLANS_G='"plans":{"codex":"ChatGPT Plus"},' ;;
...
jq -n '{model:"jev-1.13.0",answers:{rule:{type:"choice",choice:"rule_1",confidence:0.95,probabilities:{rule_1:0.95,default:0.05}},
 profile:{type:"choice",choice:"rule_1_1",confidence:0.9,probabilities:{rule_1_1:0.5,rule_1_2:0.5}}},usage:{input_tokens:1,output_tokens:1}}' > "$LAB/resp.json"
