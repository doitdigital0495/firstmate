...
4. Report status by appending one line:
   `echo "{state} [at=<epoch>]: {one short line}" >> '/tmp/tmp.QBvAlIKLQL/state/sc.status'`
...
Firstmate steers you through durable message files in '/tmp/tmp.QBvAlIKLQL/state/sc.inbox'.
When a terminal message says an instruction is waiting there - and at any natural checkpoint when you are unsure - list '/tmp/tmp.QBvAlIKLQL/state/sc.inbox'/*.msg, read and act on each message in numeric order, then acknowledge each handled message by moving it: `mv '/tmp/tmp.QBvAlIKLQL/state/sc.inbox'/NNN.msg '/tmp/tmp.QBvAlIKLQL/state/sc.inbox'/handled/`.
The move IS the acknowledgement: without it firstmate rings again and eventually treats you as stuck. An empty or absent inbox needs no action.

# Definition of done
Write your findings to `/tmp/tmp.QBvAlIKLQL/data/sc/report.md`.
...
Before reporting done, read and follow `/home/daan/.no-mistakes/worktrees/cf62ac6bfd91/01M39E053JE0HEQ96KJMGP943N/.agents/skills/captain-hold-lifecycle/SKILL.md` and pass its shared completion gate for the report and any visual review.
