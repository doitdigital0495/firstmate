...
#   than becoming intent. That library owns the parsing and intent rules. A push
#   mode (no-mistakes, direct-PR) additionally REFUSES when the brief's own
...
#   follows; scaffold such a task --mode local-only instead. That text is the
#   whole "# Task" block, so it spans both "## Captain's intent" and
#   "## Firstmate spec": the split records who authored an instruction, not
#   whether the worker must follow it, and a prohibition in either subsection
#   contradicts the mandated delivery just as hard. That check fails
...
# point. Only the read-only refusals directly above run first, so a spawn that
# was never going to proceed does not pin an unpinned home on its way out.
# A spawn is a fleet mutation, and a home driven from a
...
if ! HOME_IDENTITY_OUT=$("$SCRIPT_DIR/fm-home-identity.sh" ensure 2>&1); then
  ...
  if [ "$RELAUNCH" -ne 1 ] && [ "$("$SCRIPT_DIR/fm-home-identity.sh" show 2>/dev/null)" = absent ]; then
...
# Claude workspace-trust pre-registration (bin/fm-claude-trust.sh) is the one
# refusal in that residual: it writes the trust store for the directory the pane
# starts in, a worktree that exists only in phase 3, so its refusals - an
# out-of-scope path, a secondmate home that fails its seeded-home test, or a
# project entry that records a declined external CLAUDE.md import - also spend
# a slot.
