...
[ -n "$KEY" ] || KEY=$(fmx_env_get OPENROUTER_API_KEY "$FM_HOME/.env")
[ -n "$KEY" ] || { echo 'DISPATCH_CANARY: off (OPENROUTER_API_KEY absent)' >&2; exit 0; }
...
payload='{"model":"typesafe/jev-1.13","state":{"probe":"health"},"questions":{"probe":{"type":"choice","instructions":"Choose the only option.","criteria":{"ready":"The system is ready."}}}}'
http=$(printf '%s' "$payload" | curl -sS --max-time 5 -o "$response" -w '%{http_code}' -X POST https://openrouter.ai/api/v1/systemone \
  -H 'Content-Type: application/json' -H @/dev/fd/3 3< <(printf 'Authorization: Bearer %s\n' "$KEY") --data-binary @- 2>/dev/null) || http=000
