...
A `stale` row is keyed by the endpoint address rather than the task id, so cleanup removes the task's metadata and leaves the row queued; the row stays presentable, but the endpoint it names can never produce the event that would settle it, so the drain would re-present it every supervision turn until someone acknowledged it by sequence number.
A main drain therefore also retires, under the same queue lock and with the same bounded once-only announcement, every `stale` row whose endpoint no task metadata records any more.
Metadata absence is the authority because it is exactly what `recorded_windows()` in `bin/fm-watch.sh` uses to decide which endpoints may generate a stale row, and it needs no runtime-provider call, so the drain stays offline-safe on a path that runs every supervision turn.
Two conditions keep it conservative: a metadata scan that cannot be read retires nothing, because an unreadable `state/` must never be read as "no endpoint is recorded", and a row a live branch grant reserves is left for a later main drain once the grant releases, because a row can become unrecorded after the grant named it and main must never delete a row the branch is about to present.
...
It also pins the retired-endpoint guard: a stale row whose endpoint no task record names disappears without any `--ack-through`, is announced exactly once and never again on a later drain, while a stale row whose endpoint a task record still names stays queued and presentable.
