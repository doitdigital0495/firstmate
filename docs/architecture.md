...
`fm-spawn.sh` also owns the base-freshness boundary for every fresh ship and scout: no worker starts until its clean task worktree matches the fetched tip of origin's resolved default branch, except that a local-only ship task in a project with no origin at all starts from its verified local default-branch tip instead, and any unsafe or unverifiable base stops the spawn - PR-based modes, scouts, and a configured but unreachable or unresolvable origin are all still refused.
...
The spawn also refuses a `no-mistakes` or `direct-PR` brief whose own task text forbids the push or the pull request that its generated definition of done mandates, because that generated section is what the worker follows; `local-only` is the mode whose contract already stops at a branch the captain reviews before anything reaches a remote.
