...
The spawn also refuses a `no-mistakes` or `direct-PR` brief whose own `# Task` block forbids the push or the pull request that its generated definition of done mandates, because that generated section is what the worker follows; the whole block is read, `## Captain's intent` and `## Firstmate spec` alike, because the split records who authored an instruction rather than whether the worker must follow it; `local-only` is the mode whose contract already stops at a branch the captain reviews before anything reaches a remote.
...
The one contradictory record that retires instead is a provably older launch whose endpoint is gone, in the same record-only way and still behind the slot's dirty and landed-work inspection, so a reused slot's stale record cannot deadlock the newer owner's cleanup.
