...
Both travel in one file because Claude takes one settings source.

First verified on 2026-08-20 against Claude Code 2.1.237, hooks only, in a throwaway workspace holding its own `.claude/settings.local.json` with a `Stop` hook of its own.
...
Refreshed on 2026-09-09 against Claude Code 2.1.266 with the policy keys folded into the same file, which is the risk that fold introduces: a settings source Claude rejected would take the busy hooks down with it.

```sh
FM_CLAUDE_SETTINGS_LIVE_E2E=1 tests/fm-claude-settings-live-e2e.test.sh
ok - claude 2.1.266 (Claude Code) loads --settings from outside the workspace, merges its hooks with the project's own, and accepts the policy keys in the same file
