...
`config/` holds local gitignored operating choices, including explicit extension bindings under `config/extensions.d/`, and `projects/` holds the local project clones that Firstmate reads but changes only through the narrow guarded owner paths named in `AGENTS.md`.
...
`fm-teardown.sh <id>` takes a task id directly and validates the complete endpoint identity - from metadata alone, except for the legacy re-derivation described below - before any cleanup mutation or backend cleanup dispatch.
Missing, empty, duplicate, malformed, backend-inconsistent, or task-mismatched endpoint records are preserved and refused.
Legacy tmux and Orca metadata remains cleanup-compatible without that binding, because its recorded window name is itself `fm-<id>` and so carries the task label.
A legacy herdr, zellij, or cmux record records only opaque runtime ids, so nothing in the record binds it to its task; rather than stranding a live agent with no supported lifecycle action, `fm-teardown.sh` and `fm-control.sh` re-derive that binding from the live endpoint's own `fm-<id>` tab or workspace label, record it, and then validate normally.
The runtime query itself is read-only, and the only write the re-derivation makes before the authorization decision is the one missing binding line appended to that task's own metadata record, never an existing binding and never a project file, worktree, branch, endpoint, or process.
...
Every claude launch's `--settings` file, `state/<id>.claude-settings.json`, also carries `"attribution":{"commit":"","pr":"","sessionUrl":false}`, so a spawned worker never writes a Co-Authored-By trailer, Claude-Session link, or generated-with line into a commit or PR body regardless of which settings scopes end up loaded.
...
An omitted model or effort means the selected harness uses its own default for that axis, except where `fm-spawn.sh` pins a task-worker default, such as Pi task workers and [lean Claude task workers](#lean-claude-task-workers).
...
A candidate is rankable only with complete limit information: every window its applicable rows are bounded by - the weekly limit, a session limit where the plan has one, and named model windows - must carry a known remaining percent and reset time.
A row whose own status is not known, an unmeasured provider, a profile floor whose scope row is unmeasured, a candidate with no applicable row, or a window without a percent or reset makes that candidate incomplete and the whole choice `incomplete` before any ranking or hand-picking; a candidate is never left merely unranked, dropped, or treated as healthy because its limits are unknown.
An incomplete choice triggers a bounded gather in code: when Z.ai windows are among the missing evidence the named `zai-window-warm` pool warm-up runs once, then only the incomplete stores are re-read with doubling backoff, a failed re-read keeps the previous snapshot so the result stays `incomplete` rather than `error`, and the loop stops early when every missing measurement publishes a retry time beyond the bounded wait.
...
FM_DISPATCH_QUOTA_ATTEMPTS=3   # resolve attempts for the resolver's incomplete-quota gather loop; clamped 1..10
FM_DISPATCH_QUOTA_BACKOFF_MS=1500   # first backoff in milliseconds for that loop's doubling waits; clamped 250..30000
...
FM_COMPOSER_CAPTURE_LINES=20   # fleet-wide bound for tail-capture composer reads; tmux instead supplies its bounded visible pane, while the other adapters use this small window so stale scrollback banners stay out of the candidate set. Known miss: a tall Codex pane can hold ~20 blank rows below its idle prompt, pushing that prompt outside the window, so the read stays unknown rather than idle; raise this per host if that matters, since a wider default would let older scrollback compete with the live composer
