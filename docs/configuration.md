...
Pi reuses `reserveTokens` for a second purpose beyond the trigger: it caps the compaction summary's own output at `min(0.8 * reserveTokens, model.maxTokens)` and the turn-prefix summary at `min(0.5 * reserveTokens, model.maxTokens)`, so `97000` permits ceilings of roughly 77,600 and 48,500 tokens instead of the roughly 13,100 and 8,100 that Pi's `16384` default allows.
Those are ceilings rather than targets, still bounded by the model's own output limit, and they are accepted deliberately in order to preserve the 175,000-token trigger.
...
Pi also gates project-local `.pi/settings.json` behind project trust, recorded in `~/.pi/agent/trust.json`: an interactive session prompts once, while non-interactive runs (`-p`, `--mode json`, `--mode rpc`) never prompt and fall back to the global `defaultProjectTrust`, whose `ask` default and `never` value both ignore the file.
In an untrusted clone or a fresh worktree path the file is therefore silently ignored and Pi reverts to its `16384` default reserve, compacting near 255,600 tokens instead of 175,000 with no error to signal it; trust the project (`/trust` interactively, `--approve` for one non-interactive run, or `defaultProjectTrust: "always"`) before relying on the threshold.
