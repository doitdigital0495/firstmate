...
EOF
  : > "$launchlog"
  # Same throwaway HOME and empty CLAUDE_CONFIG_DIR as spawn_secondmate_capture:
  # that spawn pinned this home to the default Claude store
  # (bin/fm-home-identity.sh), so an ambient store here would be refused.
  PATH="$fakebin:$BASE_PATH" TMUX="fake,1,0" CLAUDECODE=1 \
    FM_ROOT_OVERRIDE="$ROOT" FM_HOME="$home" HOME="$home/user-home" CLAUDE_CONFIG_DIR='' \
...
test_spawn_secondmate_claude_permission_mode_auto() {
  ...
  launch=$(cat "$launchlog")
  assert_contains "$launch" "claude --permission-mode auto --settings '$(cd "$w/home/state" && pwd -P)/sm.claude-settings.json' --model 'opus'" \
...
SH
  ...
  [ -e "$entered" ] || fail "first config push did not reach pointer delivery: $(<"$first_out")"
