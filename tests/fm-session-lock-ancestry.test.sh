...
if [ "${FM_FIXTURE_ORPHAN_HERE:-0}" = 1 ]; then
  ...
  while [ "$i" -lt 200 ]; do
    ...
    if [ "$parent" = 1 ] || [ "$(ps -o ppid= -p "$parent" 2>/dev/null | tr -d ' ')" = 1 ]; then break; fi
    sleep 0.05
    i=$((i + 1))
  done
fi
...
# launcher exits immediately, so the tree is reparented to init or a child
# subreaper and the ancestry walk terminates inside the fixture.
# Returns once the hook has recorded its exit code.
fixture_reparented() { # <pid>
  local parent
  parent=$(ps -o ppid= -p "$1" 2>/dev/null | tr -d ' ')
  [ "$parent" = 1 ] || [ "$(ps -o ppid= -p "$parent" 2>/dev/null | tr -d ' ')" = 1 ]
}
...
test_e2e_background_session_keeps_its_lock_across_a_recycled_chain() {
  ...
  # Recycle the bridge: the daemon ends and the pty-host is reparented to init
  # or its child subreaper while the front-end keeps its lock.
  kill -TERM "$daemon"
  i=0
  while [ "$i" -lt 200 ] && { kill -0 "$daemon" 2>/dev/null || ! fixture_reparented "$ptyhost"; }; do
    sleep 0.05
    i=$((i + 1))
  done
  fixture_reparented "$ptyhost" || fail "the pty-host was not reparented after the daemon ended"
