...
# The seeded thread window is anchored to the wall clock rather than to a fixed
# date: a hard-coded expiry silently lapses and turns every rechain that checks
# the window red on a later calendar day.
iso_at() {  # <epoch>
  date -u -r "$1" +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d "@$1" +%Y-%m-%dT%H:%M:%SZ
}
REPRO_RECEIVED_EPOCH=$(date +%s)
REPRO_EXPIRES_EPOCH=$((REPRO_RECEIVED_EPOCH + 7 * 86400))
REPRO_RECEIVED_AT=$(iso_at "$REPRO_RECEIVED_EPOCH")
REPRO_EXPIRES_AT=$(iso_at "$REPRO_EXPIRES_EPOCH")
...
seed_repro_commitment() {   # <home> <obligation> <request> <work-home> <work-id>
  ...
  jq -n --arg r "$request" --arg recv "$REPRO_RECEIVED_AT" --arg exp "$REPRO_EXPIRES_AT" \
