...
test_spawn_writes_orca_metadata_and_launches_harness() {
  ...
  launch=$(cat "$staged")
  assert_contains "$launch" "CLAUDE_CODE_ENABLE_PROMPT_SUGGESTION=false CLAUDE_CODE_SEND_FEEDBACK=0 claude --dangerously-skip-permissions --settings '$(cd "$state" && pwd -P)/$id.claude-settings.json'" \
