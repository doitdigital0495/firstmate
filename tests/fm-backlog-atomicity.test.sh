...
test_ado_pr_retain_keeps_url_in_body_only() {
  local case_dir id home url out
  id=atomic-ado-retain-b7
  case_dir=$(make_home ado-retain)
  home=$(home_of "$case_dir")
  url=https://dev.azure.com/Org-1/Project/_git/Repo/pullrequest/802
  add_item "$case_dir" "$id"
  start_item "$case_dir" "$id"
  out=$(bash -c '
    . "$1/bin/fm-tasks-axi-lib.sh"
    . "$1/bin/fm-backlog-transition-lib.sh"
    fm_backlog_retain "$2/data" "$3" --pr "$4" || exit 1
    ! fm_backlog_row_artifact_supported "$3" --pr "$4"
  ' _ "$ROOT" "$home" "$id" "$url") || fail "ADO retain failed: $out"
  [ "$(row_state "$case_dir" "$id")" = 'queued' ] || fail "ADO retain did not requeue the row"
  assert_grep "Deliverable of the finished work: PR $url" "$home/data/backlog.md" \
    "ADO retain lost the deliverable line"
  pass "ADO pull request retain records the URL in the body without a row artifact"
}
