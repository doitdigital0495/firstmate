...
run_two_level() {
  ...
  # The empty CLAUDE_CONFIG_DIR matches the worker spawn below: the secondmate
  # home is pinned to the account of the session that first spawns into it
  # (bin/fm-home-identity.sh), so an ambient CLAUDE_CONFIG_DIR here would pin a
  # store the hermetic worker spawn cannot match, and that spawn would be
