...
It excludes no Claude path either, because Claude's hooks ride `--settings` from `state/`, so nothing under `.claude/` is Firstmate's to exclude; Muse scratch is likewise worker output and must refuse cleanup when uncommitted.
