...
When a newer subcommand is genuinely better where it exists, keep the older path as a fallback and cover it with a test that shims the newer one away, so CI proves the fallback on a modern git too.
