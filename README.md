...
- **Strict project boundary** - the first mate is read-only over your projects except for the narrow guarded operations authorized by [hard rule 1](AGENTS.md#1-identity-and-prime-directives), including fleet sync's guarded safe branch pruning; crewmates make every other project change behind the configured merge authority.
...
- [docs/pi-account-indicator.md](docs/pi-account-indicator.md) - the Pi footer indicator that names the Codex credential store the running session authenticates from.
