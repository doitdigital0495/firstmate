...
For Pi, approve the project trust prompt once per clone on first launch so the tracked `.pi/extensions/*.ts` files auto-load and the tracked `.pi/settings.json` applies; without that trust Pi silently ignores the settings file and its [auto-compaction threshold](docs/configuration.md#pi-auto-compaction-pisettingsjson).
